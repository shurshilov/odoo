...
class WebsiteSocial(models.Model):
    ...
    name_application = fields.Char(
        string="Name of application link",
        help="""If this is just a link leave the field empty
        And if you want it to open in the app like for example chat in whatsapp fill in the app name""",
    )
    contact = fields.Char(
        "Contacn number or account nickname",
        help="""if whataspp example your number +11231112233
    ...
    background = fields.Boolean(
        "Icon with background?", help="Add,remove class no-bg"
    )
...
class Website(models.Model):
    ...
    social_network_live = fields.Many2many(
        "website.social", string="Website live chat social network"
    )
    position_social_network = fields.Selection(
        ...
        required=True,
        default="position:fixed;height:60px;bottom:40px;right: 0px;",
    )
