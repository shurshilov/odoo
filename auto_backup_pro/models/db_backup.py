...
class DbBackup(models.Model):
    ...
    ftp_folder_id = fields.Char(
        string="FTP Folder path", help="make a folder on FTP in which you want to upload files"
    )
    ftp_email_notif_ids = fields.Many2many(
        "res.users", "ftp_user_rel", "ftp_id", "user_id", string="Person to Notify"
    )
    ...
    _constraints = [
        ...
        (
            ...
            _("Error ! No set google_drive_authorization_code! Setting it before!"),
            [],
        ),
    ]
    ...
    def google_drive_upload(self, rec, file_path, bkp_file):
        ...
        if rec.is_upload:
            ...
            para = {"name": "%s" % (str(bkp_file)), "parents": ["%s" % (str(rec.drive_folder_id))]}
            files = {
            ...
            if r.status_code == 200 and len(rec.gdrive_email_notif_ids):
                email_to = ""
