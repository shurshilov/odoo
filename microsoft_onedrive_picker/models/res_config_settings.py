...
class ResConfigSettingsOnedrive(models.TransientModel):
    ...
    onedrive_client_id = fields.Char(
        string="Microsoft Onedrive client id",
        help="Set value from your Microsoft Azure account",
    )
    onedrive_viewType = fields.Char(
        string="Microsoft Onedrive viewtype (folders, files, all)",
        default="all",
    )
    ...
    def get_values(self):
        ...
        res.update(
            onedrive_client_id=self.env["ir.config_parameter"].get_param(
                "onedrive_client_id"
            )
        )
        res.update(
            onedrive_viewType=self.env["ir.config_parameter"].get_param(
                "onedrive_viewType"
            )
        )
        res.update(
            onedrive_storage=self.env["ir.config_parameter"].get_param(
                "onedrive_storage"
            )
        )
