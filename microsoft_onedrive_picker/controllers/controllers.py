...
class OnedrivePicker(http.Controller):
    # , cors='*'
    @http.route("/onedrive_picker", auth="user", type="json", cors="*")
    def index(self, **kw):
        env = request.env["ir.config_parameter"].sudo()
        client_id = env.get_param("onedrive_client_id")
        onedrive_viewType = env.get_param(
            "onedrive_viewType"
        )  # ['https://www.googleapis.com/auth/drive']
        onedrive_storage = env.get_param("onedrive_storage")  # "image/png,image/jpeg,image/jpg"
        ...
        return {
            ...
            "onedrive_storage": onedrive_storage,
