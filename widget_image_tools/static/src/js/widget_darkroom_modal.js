...
odoo.define("web_widget_darkroom.darkroom_modal_button", function (require) {
  ...
  imageWidget.include({
    ...
    updateImage: function (result) {
      ...
      console.log(ctx);
      if (ctx.active_field === "image_medium") ctx.active_field = "image";

      return rpc
        .query({
          model: ctx.active_model,
          method: "search_read",
          args: [[["id", "=", ctx.active_record_id]], [ctx.active_field]],
          context: new Context(),
        })
    ...
    openModal: function (file_base64, clickDefault) {
      ...
      else context.size_image = "image";
      if (clickDefault) context.click = clickDefault.click;
      ...
      if (this.is_url_valid(this.value)) {
        ...
        if (this.value) {
          if (!utils.is_bin_size(this.value)) {
            ...
            if (field == "image_medium" || field == "image_small")
              field = "image";
        ...
        $(image).click(function (e) {
          var source_id =
            self.model + "/" + JSON.stringify(self.res_id) + "/image";
          var attachments = [
            {
