...
odoo.define("web_widget_darkroom.darkroom_widget", function (require) {
  ...
  FormController.include({
    _onButtonClicked: function (event) {
      if (event.data.attrs.id === "darkroom-save") {
        var self = this;
        var image_croped = self.renderer.allFieldWidgets[
          self.handle
        ][0].darkroom.sourceImage
          .toDataURL()
          .split(",")[1];
        rpc
          .query({
  ...
  var FieldDarkroomImage = imageWidget.extend({
    ...
    _init_darkroom_plugins: function (activeModal) {
      ...
      if (activeModal === "crop") cropActive = true;
      if (activeModal === "zoom") zoomActive = true;
      ...
      ButtonGroup.prototype = {
        createButton: function (options) {
          ...
          buttonElement.className =
            "btn btn-sm oe_highlight darkroom-button darkroom-button-" +
            optionsMerged.type;
          // shursh mode ADD title for buttongroup
          if (optionsMerged.image == "fa fa-search")
            this.element.innerHTML +=
              '<label class="darkroom-label">Zoom mode:</label>';
          if (optionsMerged.image == "fa fa-crop")
            this.element.innerHTML +=
              '<label class="darkroom-label">Crop mode:</label>';
          if (optionsMerged.image == "fa fa-step-backward")
            this.element.innerHTML +=
              '<label class="darkroom-label">History bar:</label>';
          if (optionsMerged.image == "fa fa-undo oe_edit_only")
            this.element.innerHTML +=
              '<label class="darkroom-label">Rotation bar:</label>';

          buttonElement.innerHTML +=
            '<i class="' + optionsMerged.image + ' fa-2x"></i>';
