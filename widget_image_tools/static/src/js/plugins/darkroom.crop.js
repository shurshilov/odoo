...
odoo.define("web_widget_darkroom.darkroom_crop", function (require) {
  ...
  var DarkroomPluginCrop = function (activeModal) {
    ...
    var CropZone = fabric.util.createClass(fabric.Rect, {
      ...
      _renderOverlay: function (ctx) {
        ...
        var x3 = Math.ceil(
          this.getWidth() / 2 +
            (canvas.width - this.getWidth() - this.getLeft()),
        );
        ...
        var y3 = Math.ceil(
          this.getHeight() / 2 +
            (canvas.height - this.getHeight() - this.getTop()),
        );
    ...
    Darkroom.plugins.crop = Darkroom.Plugin.extend({
      ...
      initialize: function InitDarkroomCropPlugin() {
        ...
        //shurh mode
        this.maxButton = buttonGroup.createButton({
          image: "fa fa-arrows-h",
          editOnly: true,
          type: "danger",
          hide: true,
        });
        ...
        this.okButton.addEventListener(
          "click",
          this.cropCurrentZone.bind(this),
        );
        ...
        this.maxButton.addEventListener(
          ...
          function () {
            ...
            canvas.bringToFront(self.cropZone);
            canvas.setActiveObject(self.cropZone);
          }.bind(this),
        );
        ...
        this.darkroom.canvas.on(
          "object:moving",
          this.onObjectMoving.bind(this),
        );
        this.darkroom.canvas.on(
          "object:scaling",
          this.onObjectScaling.bind(this),
        );
        ...
        //shursh mode click crop straight
        if (activeModal) {
          this.cropButton.element.click();
        ...
        //this._renderCropZone(0,0,this.options.maxWidth,this.options.maxHeight);
      },
      ...
      onObjectScaling: function (event) {
        ...
        //shursh mode ADD max H and W
        if (currentObject.getWidth() > this.options.maxWidth) {
          currentObject.scaleToWidth(this.options.maxWidth);
        }
        if (currentObject.getHeight() > this.options.maxHeight) {
          currentObject.scaleToHeight(this.options.maxHeight);
        }
      ...
      onKeyDown: function (event) {
        if (
          this.options.quickCropKey === false ||
      ...
      //shursh mode send coords to field in python and crop by server
      cropCurrentZone: function () {
        //params default only in ES6
        ...
        // shursh mode if call from override button with param true just send coords not real crop!
        /*                if (buttonSave===true){                        
                        var imageWidget = this.darkroom.widget;
                        var activeModel = imageWidget.field_manager.dataset.context.active_model;
                        var activeRecordId = imageWidget.field_manager.dataset.context.active_record_id;
                        var stringCoords  = ''+top+' '+left+' '+width+' '+height;
                        var coords = this.options.coords;
                        var param = {};
                        param[coords] = stringCoords;
                        if (coords)
                            new Model(activeModel).
                                call("write",[[activeRecordId],param]).
      ...
      _renderCropZone: function (fromX, fromY, toX, toY) {
        ...
        var isUp = !isDown;
        //shursh mode ADD maxheight
        var minWidth = Math.min(
          Number(this.options.minWidth),
          canvas.getWidth(),
        );
        var minHeight = Math.min(
          Number(this.options.minHeight),
          canvas.getHeight(),
        );
        var maxWidth = Math.min(
          Number(this.options.maxWidth),
          canvas.getWidth(),
        );
        var maxHeight = Math.min(
          Number(this.options.maxHeight),
          canvas.getHeight(),
        );
        ...
        //shursh mode ADD max Height and Width
        if (rightX - leftX > maxWidth) {
          if (isRight) {
            rightX = leftX + maxWidth;
          } else {
            leftX = rightX - maxWidth;
          }
        }
        if (bottomY - topY > maxHeight) {
          if (isDown) {
            bottomY = topY + maxHeight;
          } else {
            topY = bottomY - maxHeight;
          }
        }
