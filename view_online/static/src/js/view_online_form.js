  ...
  FormRenderer.include({
    ...
    start: function () {
      ...
      if (this.state && this.state.data && this.state.data.id) {
        this.call(
          "bus_service",
          "addChannel",
          this._view_online_form_channel(),
        );
        this.call(
          "bus_service",
          "onNotification",
            ...
            _.each(
              ...
              ((notification) => {
                console.log(
                  "notification :>> ",
                  notification,
                  notification[0] == this._view_online_form_channel(),
                  this._view_online_form_channel(),
                );
    ...
    _renderUsersOnline: function (usersOnline = []) {
      ...
      usersOnline.forEach((user) => {
        const icon =
          user.state == "Message.READ" || user.state == "Message.ALIVE_READ"
            ? "fa-eye"
            : "fa-edit";
        let $button = $("<button>")
          ...
          .append(
            `<span> <img width="25px" height="25px" class="rounded-circle" src="/web/image?model=res.users&amp;field=image_128&amp;id=${user.uid}"  alt="Пользователь"/> ${user.name} <i class="fa fa-1x ${icon}" style="font-size: 1em;"/></span>`,
          );
