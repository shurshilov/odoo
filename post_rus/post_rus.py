...
class sale_order(models.Model):
    ...
    def dymamic_domen(self):
        if self.payment_term_id.name == "Предоплата (100%)":
            return {"domain": {"carrier_id": [("name_del", "in", ["ПР 1-й класс", "ПР", "EMS"])]}}
        if self.payment_term_id.name == "Наложенный платеж ПР":
            return {
                "domain": {
                    "carrier_id": [
                        ("name_del", "in", ["ПР 1-й класс наложка", "ПР наложка", "EMS наложка"])
                    ]
                }
            }
...
class stock_picking(models.Model):
    ...
    def merge_all_pdf(self):
            ...
            attach_ids = self.env["ir.attachment"].search(
                [
                    (
                        "name",
                        "=",
                        "Скачать файл для печати " + str(rec.post_order_id.id_post).decode("utf-8"),
                    ),
                    ("type", "=", "binary"),
                    ("res_id", "=", rec.id),
        ...
        [
            attach_id.unlink()
            for attach_id in self.env["ir.attachment"].search([("name", "=", "TEMP POST RUS")])
        ]
        attachment = {
            "name": "TEMP POST RUS",
            "type": "binary",
    ...
    def write(self, vals):
        ...
        if self.state == "assigned" and self.carrier_id.name in [
            ...
            total_price = sum(
                [line.product_id.lst_price for line in self.pack_operation_product_ids]
            )
            percent = 0
            fix = 0
            ...
            map_delivery = {
                ...
                "ПР наложка": ["ONLINE_PARCEL", "WITH_DECLARED_VALUE_AND_CASH_ON_DELIVERY"],
                "EMS наложка": ["EMS", "WITH_DECLARED_VALUE_AND_CASH_ON_DELIVERY"],
            }
                ...
                po_id = self.env["post.order"].create(
                    {
                        ...
                        "tel_address": self.partner_id.phone,
                        "mass": str(
                            int(total_weight * 1000) if int(total_weight * 1000) > 0 else 1
                        ),
                        "insr_value": math.ceil(total_price / 100) * 100,
                        # наложенный платеж = сумма заказа - аванс(как в процентах так и фиксированный)
                        "payment": int(total_price - ((total_price * percent) / 100.0) - fix),
            ...
            attach_ids = self.env["ir.attachment"].search(
                [
                    ("name", "=", "Скачать файл для печати " + str(po_id.id_post).decode("utf-8")),
                    ("type", "=", "binary"),
                    ("res_id", "=", po_id.id),
                    ("res_model", "=", "post.order"),
                ],
...
class ResConfigSettings(models.TransientModel):
    ...
    accessToken = fields.Char(
        string="Application Authentication Token", related="company_id.accessToken"
    )
...
%(slash)s%(letter)s%(corpus)s%(building)s%(room)s%(hotel)s%(num_address_type)s"
    ...
    address_success_store = fields.Html(string="Статус адреса", help="Address success status store")
    ...
    index = fields.Char(string="Индекс", help="Zip code (index) from post of Russia")
    list_number_date = fields.Date(string="Дата документа для сдачи партии")
    date_start = fields.Date(string="Дата начала для календаря", default=fields.Date.today)
    ...
    delivery_order_id = fields.Many2one("stock.picking", string="Номер ордера доставки")
    ...
    def document_to_attachment(self, id_package, id_post, res_id, model_name):
        if id_package and id_post:
            ...
            attach_ids = self.env["ir.attachment"].search(
                [
                    ("name", "=", "Скачать архивом " + type_attach + str(id_post).decode("utf-8")),
                    ("type", "=", type_attach),
                    ("res_id", "=", res_id),
    ...
    def merger_pdf(self):
        for rec in self:
            ...
            attach_ids = self.env["ir.attachment"].search(
                [
                    (
                        "name",
                        "=",
                        "Скачать архивом " + type_attach + str(rec.id_post).decode("utf-8"),
                    ),
                    ("type", "=", type_attach),
                    ("res_id", "=", rec.id),
                    ("res_model", "=", self._name),
            ...
            for attach_id in attach_ids:
                ...
                attachment = {
                    "name": "Скачать файл для печати " + str(rec.id_post).decode("utf-8"),
                    "type": "binary",
                    "user_id": self.env.user.id,
                    "res_id": rec.id,
                    "res_model": self._name,
                ...
                for line in csv_file:
                    can.drawString(30, y, ",".join(str(x).decode("utf-8") for x in line))
                    y -= 10
                can.save()
                packet.seek(0)
    ...
    def to_new(self):
        for i in self:
            ...
            response = requests.post(
                url, headers=request_headers, data=json.dumps(shipments_to_backlogs)
            )
            if response.status_code == 200:
                i.write(
                    {
    ...
    def to_surrender(self):
        for i in self:
            ...
            if i.id_package:
                ...
                url = self.env.user.company_id.protocol + self.env.user.company_id.host + path
                response = requests.post(
                    url, headers=self._get_request_headers(), data=json.dumps(shipment_ids)
                )
    ...
    def to_archive(self):
        for i in self:
            ...
            response = requests.put(
                url, headers=request_headers, data=json.dumps(batches_to_archive)
            )
            if response.status_code == 200:
                i.write({"response": response.text + "/n TO ARCHIVE OK", "state": "archive"})
            else:
    ...
    def normalize_addr(self):
        ...
        data = [
            {"id": "adr 1", "original-address": self.address},  # "г. Москва, Варшавское шоссе, 37"
        ]
        ...
        if quality_flag and validation_flag:
            self.address_success += '<p style="color:#006400">Адрес приемлем для доставки! </p>'
