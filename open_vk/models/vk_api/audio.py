...
class VkAudio:
    ...
    def get_iter(self, owner_id=None, album_id=None):
        ...
        if album_id is not None:
            url = "https://m.vk.com/audio?act=audio_playlist{}_{}".format(owner_id, album_id)
        ...
        offset = 0
        while True:
            response = self._vk.http.get(url, params={"offset": offset}, allow_redirects=False)

            if not response.text:
                raise AccessDenied("You don't have permissions to browse user's audio")
    ...
    def get_albums_iter(self, owner_id=None):
        ...
        while True:
            ...
            if not response.text:
                raise AccessDenied(
                    "You don't have permissions to browse {}'s albums".format(owner_id)
                )
    ...
    def search_user(self, owner_id=None, q=""):
        ...
        response = self._vk.http.get(
            "https://m.vk.com/audio", params={"id": owner_id, "q": q}, allow_redirects=False
        )

        if not response.text:
            raise AccessDenied("You don't have permissions to browse {}'s audio".format(owner_id))

        return [i for i in scrap_data(response.text, self.user_id) if i["owner_id"] == owner_id]
    ...
    def search_iter(self, q, offset=0):
        ...
        while True:
            response = self._vk.http.get(
                "https://m.vk.com/audio", params={"act": "search", "q": q, "offset": offset}
            )
...
def scrap_data(html, user_id):
    ...
    for audio in soup.find_all("div", {"class": "audio_item"}):
        ...
        full_id = tuple(int(i) for i in RE_AUDIO_ID.search(audio["id"]).groups())
        ...
        albums.append(
            {
                "id": full_id[1],
                "owner_id": full_id[0],
                "url": "https://m.vk.com/audio?act=audio_playlist{}_{}".format(*full_id),
