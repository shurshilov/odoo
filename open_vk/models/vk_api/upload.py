...
class VkUpload:
    ...
    def photo_profile(self, photo, owner_id=None, crop_x=None, crop_y=None, crop_width=None):
        ...
        if crop_x is not None and crop_y is not None and crop_width is not None:
            crop_params["_square_crop"] = "{},{},{}".format(crop_x, crop_y, crop_width)
    ...
    def photo_chat(self, photo, chat_id):
        ...
        return self.vk.method("messages.setChatPhoto", {"file": response.json()["response"]})
    ...
    def audio(self, audio, artist, title):
        ...
        response.update({"artist": artist, "title": title})
    ...
    def video(
        ...
        with FilesOpener(video_file or [], "video_file") as f:
            return self.vk.http.post(url, files=f or None).json()
    ...
    def document(
        ...
        values = {"group_id": group_id, "peer_id": message_peer_id, "type": doc_type}
        ...
        response.update({"title": title, "tags": tags})
    ...
    def photo_cover(self, photo, group_id, crop_x=None, crop_y=None, crop_x2=None, crop_y2=None):
        """Загрузка изображения профиля
        ...
        url = self.vk.method("photos.getOwnerCoverPhotoUploadServer", values)["upload_url"]

        with FilesOpener(photo, key_format="file") as photo_files:
            response = self.vk.http.post(url, files=photo_files)

        return self.vk.method("photos.saveOwnerCoverPhoto", response.json())
    ...
    def story(
        ...
        if not add_to_news and not user_ids:
            raise ValueError("add_to_news and/or user_ids param is required")
