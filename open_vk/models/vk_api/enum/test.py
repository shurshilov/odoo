...
class TestEnum(unittest.TestCase):
    ...
    def test_programatic_function_unicode_iterable(self):
        SummerMonth = Enum(
            "SummerMonth", ((unicode("june"), 1), (unicode("july"), 2), (unicode("august"), 3))
        )
        lst = list(SummerMonth)
        self.assertEqual(len(lst), len(SummerMonth))
        self.assertEqual(len(SummerMonth), 3, SummerMonth)
        ...
        class auto_enum(EnumMeta):
            def __new__(metacls, cls, bases, classdict):
                ...
                for k, v in classdict.items():
                    if k not in names:
                        temp[k] = v
                return super().__new__(metacls, cls, bases, temp)
        ...
        def test_subclasses_with_getnewargs_ex(self):
            class NamedInt(int):
                ...
                def __add__(self, other):
                    temp = int(self) + int(other)
                    if isinstance(self, NamedInt) and isinstance(other, NamedInt):
                        return NamedInt("({} + {})".format(self.__name__, other.__name__), temp)
                    else:
                        return temp
    ...
    def test_inherited_new_from_enhanced_enum(self):
        ...
        self.assertEqual(
            len(Color), 3, "wrong number of elements: %d (should be %d)" % (len(Color), 3)
        )
