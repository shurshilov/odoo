...
class VkFunction:
    ...
    def __call__(self, vk, *args, **kwargs):
        ...
        return vk.method("execute", {"code": self.compile(args)}, raw=self.return_raw)
    ...
    for arg_name in kwargs.keys():
        if arg_name in function_args:
            ...
            raise VkFunctionException(
                "function got an unexpected keyword argument '{}'".format(arg_name)
            )
    ...
    if args_count != func_args_count:
        raise VkFunctionException(
            "function takes exactly {} argument{} ({} given)".format(
                func_args_count, "s" if func_args_count > 1 else "", args_count
            )
        )
