...
FORMAT_REGEX = re.compile("{}|{}|{}".format(FONT_PATTERN, COLOR_PATTERN, SIZE_REGEX))
...
class _HeaderFooterPart(Strict):
    ...
    * &nn   Selects the specified 2-digit font point size
    ...
    def __str__(self):
        ...
        fmt = []
        if self.font:
            fmt.append('&"{}"'.format(self.font))
        if self.size:
            fmt.append("&{} ".format(self.size))
        if self.color:
            fmt.append("&K{}".format(self.color))
    ...
    def from_str(cls, text):
        ...
        kw = {k: v for match in FORMAT_REGEX.findall(text) for k, v in zip(keys, match) if v}
...
class HeaderFooterItem(Strict):
    ...
    def __str__(self):
        ...
        SUBS_REGEX = re.compile("|".join(["({})".format(re.escape(k)) for k in TRANSFORM]))
        ...
        for key, part in zip(self.__keys, [self.left, self.center, self.right]):
            if part.text is not None:
                txt.append("&{}{}".format(key, str(part)))
        txt = "".join(txt)
