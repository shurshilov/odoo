...
class Worksheet(_WorkbookChild):
    ...
    def __getitem__(self, key):
        ...
        if isinstance(key, slice):
            if not all([key.start, key.stop]):
                raise IndexError("{} is not a valid coordinate or range".format(key))
            key = "{}:{}".format(key.start, key.stop)
    ...
    def unmerge_cells(
        ...
        if cr.coord not in self.merged_cells:
            raise ValueError("Cell range {} is not merged".format(cr.coord))
    ...
    def _invalid_row(self, iterable):
        raise TypeError(
            "Value must be a list, tuple, range or generator, or a dict. Supplied value is {}".format(
                type(iterable)
            )
        )
