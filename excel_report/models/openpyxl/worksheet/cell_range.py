...
class CellRange(Serialisable):
    ...
    def shift(self, col_shift=0, row_shift=0):
        ...
        if self.min_col + col_shift <= 0 or self.min_row + row_shift <= 0:
            raise ValueError(
                "Invalid shift value: col_shift={}, row_shift={}".format(col_shift, row_shift)
            )
    ...
    def issubset(self, other):
        ...
        return (other.min_row <= self.min_row <= self.max_row <= other.max_row) and (
...
class MultiCellRange(Strict):
    ...
    def __repr__(self):
        ...
        return "<{} [{}]>".format(self.__class__.__name__, ranges)
