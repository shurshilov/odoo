...
SHEETRANGE_RE = re.compile("""{}(?P<cells>{})(?=,?)""".format(SHEET_TITLE, RANGE_EXPR), re.VERBOSE)
...
def get_column_letter(
    ...
    try:
        ...
        raise ValueError("Invalid column index {}".format(idx))
...
def column_index_from_string(str_col):
    ...
    try:
        ...
        raise ValueError("{} is not a valid column name".format(str_col))
...
def range_boundaries(range_string):
    ...
    """
    msg = "{} is not a valid coordinate or range".format(range_string)
...
def rows_from_range(range_string):
    ...
    for row in rows:
        yield tuple("{}{}".format(col, row) for col in cols)
...
def cols_from_range(range_string):
    ...
    cols = (get_column_letter(col) for col in range(min_col, max_col + 1))
    for col in cols:
        yield tuple("{}{}".format(col, row) for row in rows)
