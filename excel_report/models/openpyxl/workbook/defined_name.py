...
RESERVED_REGEX = re.compile(r"^_xlnm\.(?P<name>{})".format(_names))
...
TITLES_REGEX = re.compile("""{}{}?,?{}?""".format(SHEET_TITLE, ROW_RANGE, COL_RANGE), re.VERBOSE)
...
def _unpack_print_titles(defn):
    ...
    kw = {k: v for match in scanner for k, v in match.groupdict().items() if v}
...
class DefinedNameList(Serialisable):
    ...
    def __getitem__(self, name):
        ...
        defn = self.get(name)
        if not defn:
            raise KeyError("No definition called {}".format(name))
        return defn
    ...
    def __delitem__(self, name):
        ...
        if not self.delete(name):
            raise KeyError("No globally defined name {}".format(name))
