...
class RelationshipList(Serialisable):
    ...
    def append(self, value):
        ...
        values.append(value)
        if not value.Id:
            value.Id = "rId{}".format(len(values))
...
def get_rels_path(path):
    ...
    filename = posixpath.join(folder, "_rels", "{}.rels".format(obj))
    return filename
...
def get_dependents(archive, filename):
    ...
    try:
        ...
        msg = "{} contains invalid dependency definitions".format(filename)
        warn(msg)
