...
class Typed(Descriptor):
    ...
    def __init__(self, *args, **kw):
        ...
        self.__doc__ = "Values must be of type {}".format(self.expected_type)
...
class Max(Convertible):
    ...
    def __set__(self, instance, value):
        if (self.allow_none and value is not None) or not self.allow_none:
            value = _convert(self.expected_type, value)
            if value > self.max:
                raise ValueError("Max value is {}".format(self.max))
        super().__set__(instance, value)
...
class Min(Convertible):
    ...
    def __set__(self, instance, value):
        if (self.allow_none and value is not None) or not self.allow_none:
            value = _convert(self.expected_type, value)
            if value < self.min:
                raise ValueError("Min value is {}".format(self.min))
        super().__set__(instance, value)
...
class Set(Descriptor):
    ...
    def __init__(self, name=None, **kw):
        ...
        self.__doc__ = "Value must be one of {}".format(self.values)
...
class Length(Descriptor):
    ...
    def __set__(self, instance, value):
        if len(value) != self.length:
            raise ValueError("Value must be length {}".format(self.length))
        super().__set__(instance, value)
    ...
    def __set__(self, instance, value):
        ...
        if (self.allow_none and value is not None) or not self.allow_none:
            if not self.test_pattern.match(value):
                raise ValueError("Value does not match pattern {}".format(self.pattern))
