...
*/
    _render: function () {
      ...
      if (this.attrs.options.webrtc_server && this.attrs.options.iframe) {
        var url =
          this.attrs.options.webrtc_server +
          "/webrtcstreamer.html?" +
          this.value;
        this.$el.append(
          '<div id="webrtc" class="video-container"><iframe src="' +
            url +
            '" height="300" width="400" allowfullscreen="" frameborder="0"></iframe></div><span class="togglebutton" id="toggle_webrtc">Toggle</span>',
        );
        ...
        webRtcServer = new WebRtcStreamer(
          self.$("#video")[0],
          "https://webrtc-streamer.herokuapp.com",
        );
        var url = { video: this.value };
        var options = "rtptransport=tcp&timeout=60";
        webRtcServer.connect(url.video, url.audio, options);
