...
odoo.define('dropbox_picker_widget', function (require) {
    "use strict";
    ...
    var FieldPathDropboxPicker = fields.UrlWidget.extend({
        template: 'FieldCloudDropbox',
        tagName: 'a',
        events: _.extend({}, AttachmentBox.prototype.events, {
            "click .dropbox_picker": "_onDropboxPicker",           
        }),
        ...
        _parseConfigDropbox: function (data) {
            if (data.client_id)
                this.dropbox.clientId = data.client_id;
            //if (data.onedrive_viewType)
//                this.onedrive.viewType = data.onedrive_viewType;
            if (data.dropbox_storage)
                this.dropbox.storage = data.dropbox_storage;
        },

        _copyToOdooFromDropbox: function (document){
            var deferreds = [];
            this.filesDropbox = [];
            this.blobsDropbox = [];
            // DOWNLOADING
            ...
            Promise.all(deferreds).then( res => {
                for (var i = 0; i < this.blobsDropbox.length; i++)
                       this.filesDropbox.push( new File([this.blobsDropbox[i].blob], this.blobsDropbox[i].name) );
                this.ondropDropbox(this.filesDropbox);
            });
        },

        _copyURLFromDropbox: function (document){
            this.$input.val(document.link)
            this.$el.text(document.link)
            
            this.commitChanges()
            this._setValue(this._formatValue(document.link));
            // this._rpc({
            //     model: 'ir.attachment',
            //     method: 'create',
        ...
        _onDropboxPicker: function(e) {
            this.config_read_dropbox.then( res =>{
                        // var def = $.Deferred();
                        // if (this.dropbox.storage == 'any')
                        //         this._dialogStorageDropbox(def);
                        // else
                        //     def.resolve();
                        // def.then( res =>{
        ...
        _downloadFromDropbox: async function (document) {
            var def = $.Deferred();
            fetch(document.link).then(response => response.blob())
                .then(blob => {
        ...
        _setStorageDropbox: function(value, def){
            this.dropbox.storage = value;
            def.resolve();
        },

        _dialogStorageDropbox: function(def){
            var self = this;
            new Dialog(this, {
                ...
                buttons: [
                {text: _t('Dropbox url'), classes: 'btn-primary', close: true, click: function () {self._setStorageDropbox('url', def)}},
                {text: _t('Odoo copy'), classes: 'btn-primary', close: true, click: function () {self._setStorageDropbox('copy', def)}},
                {text: _t('Cancel'), close: true}]
            }).open()
        },

        // Create and render a Picker object
        createPickerDropbox: function() {
            var def = $.Deferred();
            var options = {
    ...
    registry.add("dropbox_picker", FieldPathDropboxPicker);
});
