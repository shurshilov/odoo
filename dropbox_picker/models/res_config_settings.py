...
class ResConfigSettingsDropbox(models.TransientModel):
    ...
    dropbox_client_id = fields.Char(
        string="Dropbox client id", help="Set value from your Dropbox account"
    )
    # dropbox_viewType = fields.Char(string='Microsoft Onedrive viewtype (folders, files, all)', default='all')
    dropbox_storage = fields.Selection(
    ...
    def set_values(self):
        ...
        config_parameters.set_param("dropbox_client_id", self.dropbox_client_id)
        # config_parameters.set_param("onedrive_viewType", self.onedrive_viewType)
        config_parameters.set_param("dropbox_storage", self.dropbox_storage)
    ...
    def get_values(self):
        ...
        res.update(dropbox_client_id=self.env["ir.config_parameter"].get_param("dropbox_client_id"))
        # res.update(onedrive_viewType = self.env['ir.config_parameter'].get_param('onedrive_viewType'))
        res.update(dropbox_storage=self.env["ir.config_parameter"].get_param("dropbox_storage"))
