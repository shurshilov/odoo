...
class MailMerge:
            ...
            for file in content_types.findall("{%(ct)s}Override" % NAMESPACES):
                ...
                if type in CONTENT_TYPES_PARTS:
                    ...
                    (
                        self._settings_info,
                        self.settings,
                    ) = self.__get_tree_of_file(file)

            to_delete = []

            r = re.compile(
                r' MERGEFIELD +"?([^ ]+?)"? +(|\\\* MERGEFORMAT )', re.I
            )
            for part in self.parts.values():
                for parent in part.findall(
                    ".//{%(w)s}fldSimple/.." % NAMESPACES
                ):
                ...
                for parent in part.findall(
                    ...
                    for idx_begin, idx_end in fields:
                        ...
                        instr_elements = [
                            e
                            for e in begin.getparent().findall(
                                "{%(w)s}r/{%(w)s}instrText" % NAMESPACES
                            )
                            if idx_begin
                            < children.index(e.getparent())
                            < idx_end
                        ]
                        ...
                        if m is None:
                            continue
                        parent[idx_begin] = Element(
                            "MergeField", name=m.group(1)
                        )
        ...
        for part in parts:
            for mf in part.findall(".//MergeField"):
                for mf1 in mf.findall(".//MergeText"):
                    python_code = mf1.text.replace("MERGEFIELD", "").strip()[
                        1:-1
                    ]
                    if "MERGEFORMAT" in mf1.text:
                        python_code = (
                            mf1.text.replace("MERGEFIELD", "")
                            .replace("MERGEFORMAT", "")
                            .replace("*", "")
                            .replace("\\", "")
                            .strip()
    ...
    def merge_templates(self, replacements, separator):
        ...
        for part in self.parts.values():
            ...
            tag = root.tag
            if (
                tag == "{%(w)s}ftr" % NAMESPACES
                or tag == "{%(w)s}hdr" % NAMESPACES
            ):
                continue

            if sepClass == "section":
                # FINDING FIRST SECTION OF THE DOCUMENT
                firstSection = root.find(
                ...
                newType = etree.SubElement(
                    nextPageSec, "{%(w)s}type" % NAMESPACES
                )
        ...
        for part in parts:
            for table in part.findall(".//{%(w)s}tbl" % NAMESPACES):
                for idx, row in enumerate(table):
                    if (
                        row.find('.//MergeField[@name="%s"]' % field)
                        is not None
                    ):
