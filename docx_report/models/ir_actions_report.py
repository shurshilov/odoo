...
class IrActionsReport(models.Model):
    ...
    def _render_docx(self, res_ids, data=None):
        return self.render_docx(res_ids, data=None)
        ...
        for key in docx_list_text:
            for doc in data["docs"]:
                if key["text"][0] == '"':
                    try:
                        docx_dict.update(
                            {key["text_origin"]: str(eval(eval(key["text"])))}
                        )
                    except:
                        docx_dict.update(
                            {key["text_origin"]: str(eval(key["text"]))}
                        )
                else:
                    docx_dict.update(
                        {key["text_origin"]: str(eval(key["text"]))}
