...
class ResConfigSettingsIapFirebase(models.TransientModel):
    ...
    firebase_channel_notify = fields.Boolean(
        string="Channels(discuss) notify",
        default=True,
        help="Enable notify from discuss channels messages",
    )
    firebase_message_post_notify = fields.Boolean(
        string="Chatter(record) notify",
        default=False,
        help="Enable notify from discuss channels messages",
    )
    ...
    def get_values(self):
        ...
        res.update(
            firebase_channel_notify=self.env["ir.config_parameter"].get_param(
                "firebase_channel_notify"
            )
        )
        res.update(
            firebase_message_post_notify=self.env[
                "ir.config_parameter"
            ].get_param("firebase_message_post_notify")
        )
