...
class ResConfigSettingsIapFirebase(models.TransientModel):
    ...
    def get_values(self):
        ...
        res.update(
            res_users_firebase_title_web=self.env["ir.config_parameter"].get_param(
                "res_users_firebase_title_web"
            )
        )
        res.update(
            res_users_firebase_body_web=self.env["ir.config_parameter"].get_param(
                "res_users_firebase_body_web"
            )
        )
        res.update(
            res_users_firebase_icon_web=self.env["ir.config_parameter"].get_param(
                "res_users_firebase_icon_web"
            )
        )
        res.update(
            res_users_firebase_image_web=self.env["ir.config_parameter"].get_param(
                "res_users_firebase_image_web"
            )
        )
        res.update(
            res_users_firebase_action_web=self.env["ir.config_parameter"].get_param(
