...
class AttachmentWebcamDialog extends Component {
    ...
    async willStart() {
        let options = {
            width: this.env.session.am_webcam_width ? this.env.session.am_webcam_width : 320,
            height: this.env.session.am_webcam_height ? this.env.session.am_webcam_height : 240,
            dest_width: this.env.session.am_webcam_width ? this.env.session.am_webcam_width : 320,
            dest_height: this.env.session.am_webcam_height ? this.env.session.am_webcam_height : 240,
            image_format: 'jpeg',
            jpeg_quality: 90,
            force_flash: false,
            fps: 45,
    ...
    async onwebcam(base64, mimetype){
        let file = await this.urltoFile('data:'+mimetype+';base64,'+base64, 'snapshot.jpg', mimetype);
