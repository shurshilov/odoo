...
class Base(models.AbstractModel):
    ...
    @api.model
    def get_geoengine_layers(
        self, view_id=None, view_type="geoengine", **options
    ):
    ...
    def geo_search(
        ...
        if not search_domain:
            raise ValueError(
                _("You must at least provide one of domain or geo_domain")
            )

        return self.search(
            search_domain, limit=limit, offset=offset, order=order
        )
