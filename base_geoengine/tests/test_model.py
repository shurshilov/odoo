...
class TestModel(TransactionCase):
    ...
    def test_create_polygon_wkt_format(self):
        ...
        self.assertTrue(
            self.geo_model.geo_polygon.equals_exact(
                polygon_loaded, tolerance=0.2
            )
        )
    ...
    def test_create_point_geosjon_format(self):
        point_geosjon = '{ "type": "Point", "coordinates": [-99.2724609375, 38.25543637637949] }'
    ...
    def test_create_multi_line_wkt_format(self):
        """Create multiline"""
        multi_line = "MULTILINESTRING ((10 10, 20 20, 10 40),(40 40, 30 30, 40 20, 30 10))"
        self.geo_model.geo_multi_line = multi_line
        multi_line_loaded = wkt.loads(multi_line)
        self.assertTrue(
            self.geo_model.geo_multi_line.equals_exact(
                multi_line_loaded, tolerance=0.2
            )
        )
    ...
    def test_create_multi_point_geosjson_format(self):
        multi_point_geojson = '{"type":"MultiPoint","coordinates":[[10,40],[40,30],[20,20],[30,10]]}'
    ...
    def test_search_intersect_for_zip_1169(self):
        ...
        result = retails.search(
            [("the_point", "geo_intersect", zip_item.the_geom)]
        )
    ...
    def test_search_contains_for_retails_34(self):
        ...
        result = zip_item.search(
            [("the_geom", "geo_contains", retails.the_point)]
        )
    ...
    def test_search_within_for_retails_34(self):
        ...
        result = retails.search(
            [
                ("name", "ilike", "34"),
                ("the_point", "geo_within", zip_item.the_geom),
            ]
        )
        ...
        result = zip_item.search(
            [
                (
                    "the_poly",
                    "geo_touch",
                    {"dummy.zip.the_poly": [("id", "=", poly2.id)]},
                )
            ]
        )
    ...
    def test_search_greater_multi_polygon(self):
        ...
        result = zip_item.search(
            [
                ("city", "ilike", "Mp2"),
                ("the_geom", "geo_greater", mp1.the_geom),
            ]
        )
        self.assertEqual(1, len(result))
    ...
    def test_to_lat_lon(self):
        geo_point = (
            '{ "type": "Point", "coordinates": [601179.61612, 6399375.681364] }'
        )
