...
class AccountInvoiceWizardCopy(models.TransientModel):
    ...
    new_date_invoice = fields.Date(
        "New invoices date", required=True, default=fields.Date.today()
    )
    new_date_due = fields.Date(
        "New due Date", required=True, default=fields.Date.today()
    )
        ...
        if records:
            ...
            return {
                ...
                "view_id": self.env.ref(
                    "account_invoices_copy.account_invoice_copy_view_copy_form"
                ).id,
    ...
    def create_invoices(self):
        for invoice_record in self.env["account.invoice"].browse(
