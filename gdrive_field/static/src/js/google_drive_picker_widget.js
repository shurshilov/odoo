  ...
  var FieldPathGdrivePicker = fields.UrlWidget.extend({
            ...
            gapi.auth2.authorize(
              ...
              function (authResult) {
                if (authResult && !authResult.error) {
                  self.generate_access_token_done.resolve(
                    authResult.access_token,
                  );
                  self.saveGdriveTokenCookie(
                    authResult.access_token,
                    authResult.expires_at,
                  );
                  if (self.gdrive.pickerApiLoaded) self.createPicker();
                  ...
                  alert(
                    "Cannot get authorization token for Google Drive: " +
    ...
    createPicker: function () {
      ...
      if (this.gdrive.pickerApiLoaded && this.gdrive.oauthToken) {
        ...
        if (this.gdrive.dir) {
          picker.addView(
            new google.picker.DocsView().setParent(this.gdrive.dir),
          );
          picker.addView(
            new google.picker.DocsUploadView()
              .setParent(this.gdrive.dir)
