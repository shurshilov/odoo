...
class ResConfigSettingsFirebaseWeb(models.TransientModel):
    ...
    firebase_body_web = fields.Char(string="Firebase body web", help="Secret firebase body web-app")
    firebase_icon_web = fields.Char(string="Firebase icon web", help="Secret firebase icon web-app")
    ...
    def get_values(self):
        ...
        res.update(
            firebase_title_web=self.env["ir.config_parameter"].get_param("firebase_title_web")
        )
        res.update(firebase_body_web=self.env["ir.config_parameter"].get_param("firebase_body_web"))
        res.update(firebase_icon_web=self.env["ir.config_parameter"].get_param("firebase_icon_web"))
        res.update(
            firebase_image_web=self.env["ir.config_parameter"].get_param("firebase_image_web")
        )
        res.update(
            firebase_action_web=self.env["ir.config_parameter"].get_param("firebase_action_web")
        )
        return res
