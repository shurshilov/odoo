...
function factory(dependencies) {
  class MessagingNotificationHandler extends dependencies["mail.model"] {
    ...
    start() {
      this.env.services.bus_service.onNotification(null, (notifs) =>
        this._handleNotifications(notifs),
      );
        ...
        if (typeof message === "object") {
          switch (message.type) {
            ...
            case "mail.channel.partner/seen":
              return this._handleNotificationChannelPartnerSeen(
                message.payload,
              );
            case "mail.channel.partner/fetched":
              return this._handleNotificationChannelPartnerFetched(
                message.payload,
              );
            case "mail.channel.partner/typing_status":
              return this._handleNotificationChannelPartnerTypingStatus(
                message.payload,
              );
            ...
            case "simple_notification":
              return this._handleNotificationSimpleNotification(
                message.payload,
              );
            ...
            case "mail.channel/legacy_insert":
              return this.messaging.models["mail.thread"].insert(
                this.messaging.models["mail.thread"].convertData({
                  model: "mail.channel",
                  ...message.payload,
                }),
              );
            ...
            case "mail.guest/insert":
              return this.messaging.models["mail.guest"].insert(
                message.payload,
              );
            case "mail.message/insert":
              return this.messaging.models["mail.message"].insert(
                message.payload,
              );
            case "mail.channel.rtc.session/insert":
              return this.messaging.models["mail.rtc_session"].insert(
                message.payload,
              );
            ...
            case "res.users.settings/volumes_update":
              return this._handleNotificationVolumeSettingUpdate(
                message.payload,
              );
            default:
    ...
    _handleNotificationAttachmentDelete(payload) {
      const attachment =
        this.messaging.models["mail.attachment"].findFromIdentifyingData(
          payload,
        );
      if (attachment) {
        attachment.delete();
    ...
    _handleNotificationChannelJoined({
      ...
      if (
        ...
        invitedByUserId !== this.messaging.currentUser.id
      ) {
    ...
    async _handleNotificationChannelMessage({
      ...
      let channel = this.messaging.models[
        "mail.thread"
      ].findFromIdentifyingData({
        id: channelId,
        model: "mail.channel",
      ...
      if (!channel) {
        channel = (
          await this.async(() =>
            this.messaging.models["mail.thread"].performRpcChannelInfo({
              ids: [channelId],
            }),
          )
        )[0];
      ...
      const message =
        this.messaging.models["mail.message"].insert(convertedData);
    ...
    async _handleNotificationChannelPartnerSeen({
      ...
      const lastMessage = this.messaging.models["mail.message"].insert({
        id: last_message_id,
      });
      ...
      if (
        ...
        this.messaging.currentPartner.id === partner_id
      ) {
        channel.update({
      ...
      if (shouldComputeSeenIndicators) {
        ...
        this.messaging.models[
          "mail.thread"
        ].computeLastCurrentPartnerMessageSeenByEveryone(channel);
        // FIXME force the computing of message values (cf task-2261221)
        this.messaging.models[
          "mail.message_seen_indicator"
        ].recomputeSeenValues(channel);
      }
    }
     ...
     */
    _handleNotificationChannelPartnerTypingStatus({
      channel_id,
      is_typing,
      partner_id,
      partner_name,
    ...
    _handleNotificationChannelUpdate(channelData) {
      this.messaging.models["mail.thread"].insert({
        model: "mail.channel",
        ...channelData,
      });
    }
    ...
    async _handleNotificationRtcSessionUpdate({ id, rtcSessions }) {
      const channel = this.messaging.models[
        "mail.thread"
      ].findFromIdentifyingData({ id, model: "mail.channel" });
      if (!channel) {
        return;
      }
    ...
    _handleNotificationMessageDelete({ message_ids }) {
      for (const id of message_ids) {
        const message = this.messaging.models[
          "mail.message"
        ].findFromIdentifyingData({ id });
        if (message) {
          message.delete();
        }
      }
    ...
    _handleNotificationPartnerMarkAsRead({
      ...
      for (const message_id of message_ids) {
        ...
        const message = this.messaging.models[
          "mail.message"
        ].findFromIdentifyingData({ id: message_id });
        if (!message) {
          continue;
        }
    ...
    _handleNotificationPartnerTransientMessage(data) {
      const convertedData =
        this.messaging.models["mail.message"].convertData(data);
      const lastMessageId = this.messaging.models["mail.message"]
        .all()
        .reduce(
    ...
    async _handleNotificationPartnerUserConnection({ partnerId, username }) {
      ...
      const chat = await this.async(() =>
        this.messaging.getChat({ partnerId }),
      );
    ...
    _notifyNewChannelMessageWhileOutOfFocus({ channel, message }) {
      ...
      this.env.bus.trigger("set_title_part", {
        ...
        title: _.str.sprintf(
          titlePattern,
          messaging.outOfFocusUnreadMessageCounter,
        ),
      });
    }
  ...
  MessagingNotificationHandler.modelName =
