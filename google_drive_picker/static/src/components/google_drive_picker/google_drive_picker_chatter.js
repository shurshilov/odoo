...
import SelectStorageDialog from "@google_drive_picker/components/select_storage/select_storage";
...
patch(Chatter.prototype, {
  ...
  setup() {
    ...
    this.config_read = this.Deferred();
  ...
  generate_access_token() {
    this.generate_access_token_done = this.Deferred();
    try {
      gapi.load("auth2", {
    ...
    if (data[google.picker.Response.ACTION] == google.picker.Action.PICKED) {
      ...
      def.then((res) => {
        ...
        for (var i = 0; i < docs.length; i++) {
          ...
          if (this.gdrive.storage == "copy") this._copyToOdooFromGdrive(doc);
  ...
  _dialogStorageGdrive(def) {
    this.dialogService.add(SelectStorageDialog, {
      confirmUrl: async () => {
        this._setStorageGdrive("url", def);
      },
      confirmBinary: async () => {
        this._setStorageGdrive("copy", def);
      },
    });
  },
