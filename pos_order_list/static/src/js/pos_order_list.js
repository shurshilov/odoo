odoo.define('pos_order_list.widgets', function (require) {
    "use strict";
    ...
    var InfoPopupWidget = PopupWidget.extend({
        template: 'InfoPopupWidget',
    });
    gui.define_popup({name:'info', widget: InfoPopupWidget});
    ...
    var OrderListScreenWidget = ScreenWidget.extend({
        ...
        line_select: function (event,$line,id) {
            var self = this;
            //new Model('pos.order').call('load_done_order_for_pos', [id]).then(function (order_data) {
            return this._rpc({
                model: 'pos.order',
                ...
                _.each(orderLines, function (orderLine) {
                    ...
                    if (_.isUndefined(product)) {
                        ...
                        products.push({
                            ...
                            qty:order_data.return ? line.qty * -1 : line.qty});
                    }
                });
        ...
        render_list: function () {
            ...
            self.$('.order-list-contents').html('');
            for (var i = 0, len = Math.min(orders.length, 1000); i < len; i++) {
                var orderline_html = QWeb.render('OrderLine', {
                    widget: this,
                    order: orders[i],
                });
                self.$('.order-list-contents').append(orderline_html);
            }
        },
        ...
        load_order: function (order_id, action) {
            this.unknown_products = [];
            var self = this;
            //return new Model('pos.order').call('load_done_order_for_pos', [order_id]).then(function (order_data) {
            return this._rpc({
                model: 'pos.order',
                method: 'load_done_order_for_pos',
                args: [order_id],
        ...
        search_done_orders: function (query) {
            var self = this;
            //return new Model('pos.order').call('search_done_orders_for_pos', [query || '', this.pos.pos_session.id]).then(function (result) {
            return this._rpc({
                model: 'pos.order',
    ...
    var ListOrderButtonWidget = PosBaseWidget.extend({
        ...
        button_click: function () {
            if (this.gui.current_screen.template == 'OrderListScreenWidget'){
                this.gui.back();
                return;
            }
            this.gui.show_screen('orderlist');
