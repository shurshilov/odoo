odoo.define(
  "odoo_website_search_suggestion.odoo_website_search_suggestion",
  function (require) {
    "use strict";
        ...
        if (utils.get_cookie("history_search")) {
          ...
          for (let i = 0; i < history.length; i++) {
            ...
            res["products"].push({
              ...
              price:
                '<span class="fa fa-times o_history_delete_cross" data-name="' +
                history[i] +
                '"></span>',
            });
          }
          ...
          $(".o_history_delete_cross").click(function (e) {
            ...
            for (let i = 0; i < history.length; i++) {
              if (history[i] !== name_del) {
                if (i !== history.length - 1) res += history[i] + "|";
                else res += history[i];
              }
            }
    ...
    publicWidget.registry.WebsiteSale.include({
      _onSubmitSaleSearch: function (ev) {
        ...
        if (!ev.isDefaultPrevented() && !$this.is(".disabled")) {
          ...
          else utils.set_cookie("history_search", search.val());
          window.location =
            oldurl +
            "&" +
            search.attr("name") +
