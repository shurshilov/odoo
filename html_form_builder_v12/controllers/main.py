...
class HtmlFormController(http.Controller):
    def _html_action_custom_server_action(self, submit_action, history_data, values):
        form_record = request.env[history_data.html_id.model_id.model].browse(
            history_data.record_id
        )
        submit_action.custom_server_action.with_context(
            {"active_id": form_record.id, "active_model": history_data.html_id.model_id.model}
        ).run()

    @http.route("/form/thankyou", type="http", auth="public", website=True)
    def html_thanks(self, **kw):
        return http.request.render("html_form_builder_v12.html_thank_you", {})
    ...
    def form_field_config_input_group(self, **kw):
        ...
        for field in request.env["ir.model.fields"].search(
            [("model_id.model", "=", my_field.relation), ("name", "!=", "display_name")]
        ):
            if (
                field.ttype == "char"
                or field.ttype == "integer"
                or field.ttype == "selection"
                or field.ttype == "binary"
            ):
                field_options_html += (
                    '<input type="checkbox" name="input_group_fields" value="'
                    + str(field.id)
                    + '"/> '
                    + str(field.field_description)
                    + " ("
                    + str(field.ttype)
                    + ")<br/>\n"
                )
    ...
    def form_field_config_general(self, **kw):
        ...
        request.env["html.form"].browse(int(values["form_id"]))

        field_options_html = ""
        field_options_html = '<option value="">Select Field</option>'

        for my_field in request.env["ir.model.fields"].search(
            [
                ("ttype", "in", data_types_list),
                ("model_id.model", "=", values["form_model"]),
                ("name", "!=", "display_name"),
            ]
        ):
            field_options_html += (
    ...
    def html_captcha(self, **kw):
        ...
        html_string += (
            ...
            + str(html_form.captcha_client_key)
            + '"></div>'
        )
    ...
    def _generate_html_date_picker(self, field):
        ...
        html_output += (
            '<div class="hff hff_date_picker form-group" data-form-type="'
            + field.field_type.html_type
            + '" data-field-id="'
            + str(field.id)
            + '">\n'
        )
    ...
    def _generate_html_datetime_picker(self, field):
        ...
        html_output += (
            '<div class="hff hff_datetime_picker form-group" data-form-type="'
            + field.field_type.html_type
            + '" data-field-id="'
            + str(field.id)
            + '">\n'
    ...
    def _generate_html_textbox(self, field):
        ...
        html_output = ""
        html_output += (
            '<div class="hff hff_textbox form-group" data-form-type="'
            + field.field_type.html_type
            + '" data-field-id="'
            + str(field.id)
            + '">\n'
        ...
        if field.validation_format == "integer":
            html_output += r' pattern="\d*" title="Whole Numbers Only" min="0" step="1"'
    ...
    def _generate_html_textarea(self, field):
        ...
        html_output = ""
        html_output += (
            '<div class="hff hff_textarea form-group" data-form-type="'
            + field.field_type.html_type
            + '" data-field-id="'
            + str(field.id)
            + '">\n'
    ...
    def _generate_html_file_select(self, field):
        ...
        html_output += (
            '<div class="hff hff_binary form-group" data-form-type="'
            + field.field_type.html_type
            + '" data-field-id="'
            + str(field.id)
            + '">\n'
    ...
    def _generate_html_input_group(self, field):
        ...
        html_output += (
            '      $( "#'
            + field.html_name
            + '_container" ).append( \'<div class="row form-group">\' + $( "#'
            + field.html_name
        ...
        html_output += (
            '<div class="hff hff_input_group form-group" data-sub-field-number="'
            + str(sub_field_number)
            + '" data-html-name="'
            + field.html_name
            + '" data-form-type="'
            + field.field_type.html_type
        ...
        html_output += (
            '    <div id="' + str(field.html_name) + '_placeholder" class="row form-group">\n'
        )
        ...
        for sub_field in field.setting_input_group_sub_fields:
            if sub_field.ttype == "char" or sub_field.ttype == "integer":
                html_output += (
                    '      <div class="col-md-'
                    + str(column_width)
                    + '"><input type="text" class="form-control" data-sub-field-name="'
                    + str(sub_field.name)
                    + '" placeholder="'
                    + str(sub_field.field_description)
                    + '"/></div>\n'
                )
            elif sub_field.ttype == "selection":
                html_output += '      <div class="col-md-' + str(column_width) + '">\n'
                html_output += (
                    '<select class="form-control" data-sub-field-name="'
                    + str(sub_field.name)
                    + '" placeholder="'
                    + str(sub_field.field_description)
                    + '">\n'
                ...
                selection_list = dict(
                    request.env[sub_field.model_id.model]._fields[sub_field.name].selection
                )
                ...
                for selection_value, selection_label in selection_list.items():
                    html_output += (
                        '<option value="' + selection_value + '">' + selection_label + "</option>\n"
                    )
                ...
                html_output += (
                    '      <div class="col-md-'
                    + str(column_width)
                    + '"><input type="file" data-sub-field-name="'
                    + str(sub_field.name)
                    + '" placeholder="'
                    + str(sub_field.field_description)
                    + '"/></div>\n'
        ...
        html_output += (
            '    <button class="btn btn-primary btn-md row pull-right" id="'
            + field.html_name
            + '_add">Add (+)</button>\n'
        )
        html_output += "  </div>\n"
    ...
    def _generate_html_radio_group_selection(self, field):
        ...
        html_output = ""
        html_output += (
            '<div class="hff hff_radio_group form-group" data-form-type="'
            + field.field_type.html_type
            + '" data-field-id="'
            + str(field.id)
            + '">\n'
        ...
        selection_list = dict(
            request.env[field.field_id.model_id.model].fields_get(allfields=[field.field_id.name])[
                field.field_id.name
            ]["selection"]
        )
        ...
        for selection_value, selection_label in selection_list.items():
            if (
                field.setting_radio_group_layout_type == "multi"
                or not field.setting_radio_group_layout_type
            ):
    ...
    def _generate_html_checkbox_group(self, field):
        ...
        html_output = ""
        html_output += (
            '<div class="hff hff_checkbox_group" data-form-type="'
            + field.field_type.html_type
            + '" data-field-id="'
            + str(field.id)
            + '">\n'
    ...
    def _generate_html_checkbox_boolean(self, field):
        ...
        html_output = ""
        html_output += (
            '<div class="hff hff_checkbox checkbox" data-form-type="'
            + field.field_type.html_type
            + '" data-field-id="'
            + str(field.id)
            + '">\n'
    ...
    def _generate_html_dropbox(self, field):
        ...
        html_output += (
            '<div class="hff hff_dropbox form-group" data-form-type="'
            + field.field_type.html_type
            + '" data-field-id="'
            + str(field.id)
            + '">\n'
        ...
        html_output += (
            '  <label class="control-label" for="' + field.html_name + '">' + field.field_label
        )
        html_output += "</label>\n"
        ...
        if field.field_id.ttype == "selection":
            selection_list = dict(
                request.env[field.field_id.model_id.model]._fields[field.field_id.name].selection
            )
    ...
    def load_form(self, **kw):
        ...
        if html_form.captcha:
            form_string += (
                '<div class="html_form_captcha col-md-12 form-group" data-captcha-id="'
                + str(html_form.captcha.id)
                + '">\n'
            )
        ...
        form_string += "      </div>\n"
        form_string += (
            '      <input type="hidden" name="form_id" value="' + str(html_form.id) + '"/>\n'
        )
    ...
    def new_form(self, **kw):
        ...
        html_form = request.env["html.form"].create(
            {"name": "My New Form", "model_id": my_model.id}
        )
        ...
        return {
            ...
            "form_model": action.action_model,
            "form_id": html_form.id,
        }
    ...
    def form_fieldtype(self, **kw):
        ...
        field_type = request.env["html.form.field.type"].search(
            [("html_type", "=", values["field_type"])]
        )[0]
    ...
    def form_add_field(self, **kw):
        ...
        if (
            len(
                request.env["html.form.field"].search(
                    [("html_id", "=", int(values["form_id"])), ("field_id", "=", field_id.id)]
                )
            )
            > 0
        ):
        ...
        if "sub_fields" in values:
            insert_values["setting_input_group_sub_fields"] = [
                (6, 0, [int(i) for i in values["sub_fields"]])
            ]
    ...
    def process_form(self, kwargs):
        ...
        if (
            http.request.env["html.form"].sudo().search_count([("id", "=", int(values["form_id"]))])
            == 0
        ):
            ...
            if fi.html_name in values:
                ...
                if field_valid.error == "":
                    ...
                    insert_data_dict.append(
                        {"field_id": fi.field_id.id, "insert_value": field_valid.history_data}
                    )
                else:
        ...
        if form_error:
            ...
            try:
                new_record = (
                    http.request.env[entity_form.model_id.model].sudo().create(secure_values)
                )
