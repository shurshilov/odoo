...
class HtmlForm(models.Model):
    ...
    defaults_values = fields.One2many(
        ...
        help="Sets the value of an field before it gets inserted into the database",
    )
    return_url = fields.Char(
        string="Return URL",
        default=_default_return_url,
        help="The URL that the user will be redirected to after submitting the form",
        required=True,
    )
    ...
    def _generate_html_checkbox_boolean(self, fe):
        ...
        html_output += "  <label for='" + fe.html_name + "'>" + fe.field_label + "</label>\n"
        html_output += (
            '  <input type="checkbox" id="' + fe.html_name + '" name="' + fe.html_name + '"'
        )
    ...
    def _generate_html_radio_group_selection(self, fe):
        ...
        selection_list = dict(
            self.env[fe.field_id.model_id.model]._fields[fe.field_id.name].selection
        )
...
class HtmlFormActionType(models.Model):
    ...
    internal_name = fields.Char(
        ...
        help="action is executed in controller '_html_action_<internal_name>'",
    )
...
class HtmlFormField(models.Model):
    ...
    field_id = fields.Many2one(
        "ir.model.fields",
        domain="[('name','!=','create_date'),('name','!=','create_uid'),('name','!=','id'),('name','!=','write_date'),('name','!=','write_uid'),('name','!=','display_name')]",
        string="Form Field",
    )
    ...
    setting_radio_group_layout_type = fields.Selection(
        [("single", "Single"), ("multi", "Multi")], string="Layout Type"
    )
    setting_date_format = fields.Selection(
        [("days", "Days"), ("months", "Months"), ("years", "Years")], string="Date Format"
    )
    setting_datetime_format = fields.Selection(
        [("days", "Days"), ("months", "Months"), ("years", "Years")], string="Datetime Format"
    )
    ...
    def _onchange_field_id(self):
        ...
        if self.field_id:
            self.field_type = (
                self.env["html.form.field.type"]
                .search([("data_type", "=", self.field_id.ttype), ("default", "=", True)])[0]
                .id
            )
...
class HtmlFormDefaults(models.Model):
    ...
    default_value = fields.Char(
        ...
        help="use 'user_id' to get the current website user, 'partner_id' for the user partner record",
    )
...
class HtmlFormFieldType(models.Model):
    ...
    default = fields.Boolean(
        string="Default", help="Is this the default HTML type for this datatype?"
    )
...
class HtmlFormHistory(models.Model):
    ...
    insert_data = fields.One2many(
        "html.form.history.field", "html_id", string="HTML Fields", readonly=True
    )
