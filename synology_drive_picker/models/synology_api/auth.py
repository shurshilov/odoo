...
class Authentication:
    ...
    def login(self, application):
        ...
        if not self._session_expire:
            ...
            session_request = requests.get(
                self._base_url + login_api, param, verify=False
            )
    ...
    def get_api_list(self, app=None):
        ...
        response = requests.get(
            self._base_url + query_path, list_query, verify=False
        ).json()
    ...
    def request_data(
        self, api_name, api_path, req_param, method=None, response_json=True
    ):  # 'post' or 'get'
