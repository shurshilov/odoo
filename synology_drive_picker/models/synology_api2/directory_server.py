...
class DirectoryServer(base_api.BaseApi):
    ...
    def list_directory_objects(
        ...
        req_param = {
            ...
            "limit": limit,
            "method": method,
            "objectCategory": json.dumps(objectCategory),
            "offset": offset,
            "scope": scope,
            "version": info["maxVersion"],
        }
    ...
    def change_user_password(
        ...
        compound = (
            '[{"api":"SYNO.ActiveDirectory.User","method":"set","version":2,"userList":[{"dn":"'
            + user_dn
            + '","enbl_change_password":true,"password":"'
            + password
            + '"}]}]'
        )
        api_path = info["path"]
    ...
    def add_user_to_group(
        ...
        api_name = "SYNO.Entry.Request"
        compound = (
            '[{"api":"SYNO.ActiveDirectory.Group.Member","method":"add","version":"1","dn":"'
            + groupDn
            + '","members":["'
            + userDn
            + '"]}]'
    ...
    def deleteItems(self, dnList: list[str]) -> dict[str, object] | str:
        ...
        req_param = {
            "api": api_name,
            "method": "delete",
            "dnList": json.dumps(dnList),
            "version": 2,
        }
        task_id = self.request_data(api_name, api_path, req_param)["data"][
            "task_id"
        ]
    ...
    def entryRequest(self, task_id: str) -> Any:
        ...
        compound = (
            '[{"api":"SYNO.ActiveDirectory.Polling","method":"get","version":1,"task_id":"'
            + task_id
            + '"}]'
        )
        api_path = info["path"]
