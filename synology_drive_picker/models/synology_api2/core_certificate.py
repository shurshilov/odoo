...
class Certificate(base_api.BaseApi):
    ...
    def _base_certificate_methods(
        ...
        req_param = {"version": "1", "method": method}

        if "set" == method and cert_id:
            req_param.update(
                {"as_default": "true", "desc": '""', "id": f'"{cert_id}"'}
            )
    ...
    def upload_cert(
        ...
        ) + "?api=%s&version=%s&method=import&_sid=%s" % (
            api_name,
            info["minVersion"],
            self._sid,
        )
        ...
        data_payload = {
            "id": cert_id or "",
            "desc": desc or "",
            "as_default": "true" if set_as_default else "false",
        }

        with open(serv_key, "rb") as payload_serv_key, open(
            ser_cert, "rb"
        ) as payload_ser_cert:
            files = {
                "key": (
            ...
            if ca_cert:
                with open(ca_cert, "rb") as payload_ca_cert:
                    ...
                    r = session.post(
                        ...
                        files=files,
                        data=data_payload,
                        verify=self.session.verify_cert_enabled(),
    ...
    def export_cert(self, cert_id: str) -> BytesIO | None:
