...
class FileStation(base_api.BaseApi):
    ...
    def get_file_info(
        self, path: str | None = None, additional: str | list[str] | None = None
    ) -> dict[str, object] | str:
        api_name = "SYNO.FileStation.List"
        info = self.file_station_list[api_name]
        api_path = info["path"]
    ...
    def search_start(
        ...
        mtime_from: str | int | None = None,
        mtime_to: str | int | None = None,
        crtime_from: str | int | None = None,
        crtime_to: str | int | None = None,
        atime_from: str | int | None = None,
        atime_to: str | int | None = None,
        owner: str | None = None,
        group: str | None = None,
    ) -> dict[str, object] | str:
        ...
        for key, val in locals().items():
            if (
                key not in ["self", "api_name", "info", "api_path", "req_param"]
                and "time" not in key
            ):
                if val is not None:
                    req_param[str(key)] = val
        ...
        message = (
            "You can now check the status of request with "
    ...
    def get_search_list(
        ...
        if task_id is None:
            return (
                "Enter a correct taskid, choose one of the following: "
    ...
    def stop_search_task(self, taskid: str) -> dict[str, object] | str:
        ...
        if taskid is None:
            return "Enter a valid taskid, choose between " + str(
    ...
    def stop_all_search_task(self) -> str:
        ...
        assert len(self._search_taskid_list), "Task list is empty" + str(
            self._search_taskid_list
        )
    ...
    def add_a_favorite(
        self, path: str, name: str | None = None, index: int | None = None
    ) -> dict[str, object] | str:
        api_name = "SYNO.FileStation.Favorite"
        info = self.file_station_list[api_name]
        api_path = info["path"]
    ...
    def replace_all_favorite(
        self, path: str | list[str], name: str | list[str]
    ):
        api_name = "SYNO.FileStation.Favorite"
        info = self.file_station_list[api_name]
        api_path = info["path"]
    ...
    def start_dir_size_calc(self, path: str) -> dict[str, object] | str:
        ...
        taskid = self.request_data(api_name, api_path, req_param)["data"][
            "taskid"
        ]
        ...
        message = (
            "You can now check the status of request "
            "with get_dir_status() , your id is: " + response_id
        )
        if self.interactive_output:
            output = message
        else:
    ...
    def get_md5_status(
        ...
        if taskid is None and self._md5_calc_taskid != "":
            ...
            return (
                "Did you run start_md5_calc() first? No task id found! "
                + str(self._md5_calc_taskid)
            )
    ...
    def upload_file(
        ...
        with open(file_path, "rb") as payload:
            url = (
                "%s%s" % (self.base_url, api_path)
            ) + "?api=%s&version=%s&method=upload&_sid=%s" % (
                api_name,
                info["minVersion"],
                self._sid,
            )
    ...
    def create_sharing_link(
        ...
        date_expired: str | int | None = None,
        date_available: str | int | None = None,
        expire_times: int = 0,
    ) -> dict[str, object] | str:
        api_name = "SYNO.FileStation.Sharing"
        info = self.file_station_list[api_name]
    ...
    def start_copy_move(
        ...
        overwrite: bool | None = None,
        remove_src: bool | None = None,
        accurate_progress: bool | None = None,
        search_taskid: str | None = None,
    ) -> str | dict[str, object]:
        ...
        message = (
            "You can now check the status of request with "
            "get_copy_move_status() , your id is: " + self._copy_move_taskid
        )
        if self.interactive_output:
            output = message
        else:
    ...
    def start_delete_task(
        ...
        self._delete_taskid = self.request_data(api_name, api_path, req_param)[
            "data"
        ]["taskid"]
        ...
        message = (
            "You can now check the status of request with "
            "get_delete_status() , task id is: " + self._delete_taskid
        )
        if self.interactive_output:
            output = message
        else:
    ...
    def start_extract_task(
        ...
        overwrite: bool | None = None,
        keep_dir: bool | None = None,
        create_subfolder: bool | None = None,
        ...
        self._extract_taskid = self.request_data(api_name, api_path, req_param)[
            "data"
        ]["taskid"]
        ...
        message = (
            "You can now check the status of request with "
            "get_extract_status() , your id is: " + self._extract_taskid
        )
        if self.interactive_output:
            output = message
        else:
    ...
    def start_file_compression(
        ...
        message = (
            "You can now check the status of request with "
            "get_compress_status() , your id is: " + self._compress_taskid
        )
        if self.interactive_output:
            output = message
        else:
    ...
    def get_list_of_all_background_task(
        self,
        offset: int | None = None,
        limit: int | None = None,
        sort_by: str | None = None,
