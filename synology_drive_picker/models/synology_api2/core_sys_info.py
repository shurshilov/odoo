...
class SysInfo(base_api.BaseApi):
    ...
    def set_fan_speed(
        self, fan_speed: str = "quietfan"
    ) -> dict[str, object] | str:  # coolfan , fullfan
        api_name = "SYNO.Core.Hardware.FanSpeed"
        info = self.core_list[api_name]
        api_path = info["path"]
        req_param = {
    ...
    def enable_zram(self, enable_zram: bool = True) -> dict[str, object] | str:
        ...
        req_param = {
            "version": info["maxVersion"],
            "method": "set",
            "enable_zram": str(enable_zram).lower(),
        }

        return self.request_data(api_name, api_path, req_param)

    def enable_power_recovery(
        self, restart_auto_after_issue: bool = True, wake_on_lan: bool = False
    ) -> dict[str, object] | str:
        ...
        req_param = {
            "version": info["maxVersion"],
            "method": "set",
            "rc_power_config": str(restart_auto_after_issue).lower(),
            "wol1": str(wake_on_lan).lower(),
        }
    ...
    def enable_beep_control(
        self,
        fan_fail: bool | None = None,
        volume_crash: bool | None = None,
        poweron_beep: bool | None = None,
        poweroff_beep: bool | None = None,
    ) -> dict[str, object] | str:
        api_name = "SYNO.Core.Hardware.BeepControl"
        info = self.core_list[api_name]
        api_path = info["path"]
        req_param = {
            "version": info["maxVersion"],
            "method": "set",
            "fan_fail": str(fan_fail).lower(),
            "volume_crash": str(volume_crash).lower(),
            "poweron_beep": str(poweron_beep).lower(),
    ...
    def set_hibernation(
        self, internal_hd_idletime: int = 0, usb_idletime: int = 0
    ) -> dict[str, object] | str:
    ...
    def enable_external_ups(
        ...
        req_param = {
            ...
            "snmp_auth_key_dirty": str(snmp_auth_key_dirty).lower(),
    ...
    def shutdown(self, version: str = None) -> dict[str, object] | str:
        ...
        api_path = info["path"]
        req_param = {
            "version": (info["maxVersion"] if version is None else version),
            "method": "shutdown",
        }
    ...
    def gateway_list(
        self, ip_type: str = "ipv4", type: str = "wan"
    ) -> dict[str, object] | str:
