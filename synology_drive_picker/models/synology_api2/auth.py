...
class Authentication:
            ...
            if USE_EXCEPTIONS:
                try:
                    session_request = requests.get(
                        self._base_url + login_api, params, verify=self._verify
                    )
    ...
    def request_multi_datas(
        self,
        compound: dict[object] = None,
        method: str | None = None,
        mode: str | None = "sequential",  # "sequential" or "parallel"
        ...
        if error_code:
            ...
            if USE_EXCEPTIONS:
                ...
                # DHCP Server error:
                    ...
                    api_name.find("DHCPServer") > -1
                    or api_name == "SYNO.Core.TFTP"
                ):
                    raise DHCPServerError(error_code=error_code)
                # Active Directory error:
                elif api_name.find("ActiveDirectory") > -1 or api_name in (
                    "SYNO.Auth.ForgotPwd",
                    "SYNO.Entry.Request",
                ):
                ...
                # Synology drive admin error:
                    ...
                    api_name.find("SynologyDrive") > -1
                    or api_name == "SYNO.C2FS.Share"
                ):
