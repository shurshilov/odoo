...
class _Schedule:
    ...
    def _generate_dict(self) -> dict:
        schedule_dict = {
            ...
            "last_work_hour": self.same_day_repeat_until
            if self.same_day_repeat_until > -1
            else self.start_time_h,  # Last run time, defaults to start time if not provided
        }
        repeat_modality = -1
...
class TaskScheduler(base_api.BaseApi):
    ...
    def __get_root_token(self) -> str:
        sys_info = SysInfo(
            ip_address=self.session._ip_address,
            port=self.session._port,
            username=self.session._username,
            password=self.session._password,
            secure=self.session._secure,
            cert_verify=self.session._verify,
            dsm_version=self.session._version,
            debug=self.session._debug,
            otp_code=self.session._otp_code,
    ...
    def get_task_config(
        self, task_id: int, real_owner: str, type: str = ""
    ) -> dict[str, object] | str:
    ...
    def get_task_results(self, task_id: int) -> dict[str, object] | str:
    ...
    def set_output_config(
        self, enable_output: bool, output_path: str = ""
    ) -> dict[str, object] | str:
    ...
    def task_set_enable(
        self, task_id: int, real_owner: str, enable: bool
    ) -> dict[str, object] | str:
        ...
        """
        task_dict = {"id": task_id, "real_owner": real_owner, "enable": enable}
    ...
    def task_run(
        self, task_id: int, real_owner: str
    ) -> dict[str, object] | str:
        """Run a specific task.
        ...
        """
        task_dict = {"id": task_id, "real_owner": real_owner}
    ...
    def create_beep_control_task(
        ...
        extra = {"beep_duration": str(beep_duration)}
    ...
    def create_recycle_bin_task(
        ...
        extra = {
            "clean_share_policy": {"clean_all": clean_all_shares},
