...
class MainController(http.Controller):
    @http.route(
        "/website_adv_image_optimization/attachment/optimize",
        type="http",
        auth="user",
        methods=["POST"],
    )
    def optimize(self, attachment_id, quality=80, width=False, height=False, **kwargs):
        ...
        if (
            ...
            and width > 0
            and height > 0
            and (width != image.size[0] or height != image.size[1])
        ):
        ...
        attachment.write({"datas": base64.b64encode(data), "name": new_name})
