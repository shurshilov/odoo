...
class FaceIdNotification(models.Model):
    ...
    state = fields.Selection([
        ('off', 'Off'),
        ('on', 'On')
    ], string='State (enable/disable)', default='off')
    type = fields.Selection([
        ('email', 'Email'),
        ('sms', 'Sms')
    ], string='Type notification', default='email')
    action = fields.Selection([
        ('off', 'Camera off'),
        ('black', 'Face from Black list found'),
        ('employee', 'Employee found')
    ], string='Action to send', default='off')
    template_id = fields.Many2one(
        'mail.template', string='Email Template',
        domain=[('model', '=', 'faceid.notification')], ondelete='restrict',
        help='This field contains the template of the mail that will be automatically sent', required=True)
...
class FaceIdAction(models.Model):
    ...
    def create(self, values):
        rec = super(FaceIdAction, self).create(values)
...
class FaceIdSource(models.Model):
    ...
    faces = fields.One2many("faceid.action", "source_id", string="Found actions", domain=[('create_date','>=',datetime.today().strftime("%Y-%m-%d"))])
    ...
    state_compute = fields.Selection([
        ('off', 'Off'),
        ('on', 'On')
    ], string='State compute', compute="_compute_state")
    state = fields.Selection([
        ('off', 'Off'),
        ('on', 'On')
    ], string='Status', readonly=True, copy=False, default='off', store=True, related='state_compute')
    faces_count_today = fields.Integer(compute='_compute_faces_count_today', string="Faces Count", type='integer')
    faces_count_all = fields.Integer(compute='_compute_faces_count_all', string="Faces Count", type='integer')
    ...
    def _compute_faces_count_today(self):
        FaceidAction = self.env['faceid.action']
        for rec in self:
            rec.faces_count_today = FaceidAction.search_count([('source_id', '=', rec.id),('create_date','>=',datetime.now().strftime("%Y-%m-%d"))])
            ...
            if(i % rec.frame_interval == 0):
                #try:
                    ...
                    if preprocessor.filename != '1.png':
                        ...
                        self._cr.commit()
                        rec.faces.filtered(lambda r: r.create_date.strftime("%Y-%m-%d") == datetime.now().strftime("%Y-%m-%d"))
                        ...
                        min_dist, id = classifier.predict_odoo(preprocessor.scaled, list_images , _logger, list_ids)
                        #classifier.sess.close()
                        _logger.warning("min_dist2")
                        _logger.warning(min_dist)
                        # [new] если похожего лица нет в базе
                        if min_dist >= threshold:
                            rec.save_face_odoo(preprocessor.filename, preprocessor.scaled, min_dist, -1)
                        else:
                            rec.save_face_odoo(preprocessor.filename, preprocessor.scaled, min_dist, id)
                        ...
                        for rec in env['faceid.source'].search([('id','in',ids)]):
                            ...
                            while(recall):
                                ...
                                if camera.isOpened():
                                    ...
                                    _logger.warning("Camera not opened " + datetime.now().strftime("%m-%d-%Y %I:%M:%S%p"))
                                    camera.release()
                                    attempts -= 1
