...
from . import classify, preprocess
...
class FaceIdNotification(models.Model):
    ...
    state = fields.Selection(
        [("off", "Off"), ("on", "On")], string="State (enable/disable)", default="off"
    )
    type = fields.Selection(
        [("email", "Email"), ("sms", "Sms")], string="Type notification", default="email"
    )
    ...
    source_id = fields.Many2one(comodel_name="faceid.source", string="Source link", required=True)
    count_send_setting = fields.Integer(string="Send Counter settings", default=1)
    ...
    def send_email(self):
        ...
        # for rec in self:
        #    if rec.state == 'on' and rec.type == 'email':
        for rec in self.search([("state", "=", "on"), ("type", "=", "email")]):
            ...
            if rec.count_send_real > 0:
                ...
                if not len([s for s in [str(i) for i in threading.enumerate()] if name in s]):
                    # send message camera_off
                    rec.count_send_real = rec.count_send_real - 1
                    template_browse = self.env["mail.template"].browse(rec.template_id.id)
...
class FaceIdAction(models.Model):
    ...
    face_id = fields.Many2one(comodel_name="faceid.action", string="Ближайшая запись")
    face_id_image = fields.Binary(related="face_id.face", string="Ближайшее лицо фото")
    min_distanse = fields.Float(string="Min distanse", default=101)
    unique = fields.Boolean(string="Unique", compute="_compute_unique", store=True)
    source_id = fields.Many2one(comodel_name="faceid.source", string="Source link")
    ...
    @api.multi
    def write(self, vals):
        if vals.get("min_distanse", False):
            # if vals.get('min_distanse', False) >= rec.source_id.recognize_threshold:
            if vals.get("min_distanse", False) >= 0.80:
                vals["unique"] = True
            else:
                vals["unique"] = False
...
class FaceIdSource(models.Model):
    ...
    x = fields.Integer(string="X coords crop", default=0)
    y = fields.Integer(string="Y coords crop", default=0)
    height = fields.Integer(string="height crop", default=0)
    width = fields.Integer(string="width crop", default=0)
    height_source = fields.Integer(string="height source", default=0)
    width_source = fields.Integer(string="width source", default=0)
    speed_detect = fields.Integer(string="Speed face detect, ms", default=0)
    recognize_threshold = fields.Float(string="Recognize threshold", default=0.8)
    state_compute = fields.Selection(
        [("off", "Off"), ("on", "On")], string="State compute", compute="_compute_state"
    )
    state = fields.Selection(
        [("off", "Off"), ("on", "On")],
        string="Status",
    ...
    faces_count_today = fields.Integer(
        compute="_compute_faces_count_today", string="Faces Count", type="integer"
    )
    faces_count_all = fields.Integer(
        compute="_compute_faces_count_all", string="Faces Count", type="integer"
    )
    unique_faces_count_today = fields.Integer(
        compute="_compute_unique_faces_count_today", string="Faces Count unique", type="integer"
    )
    ...
    def _compute_faces_count_all(self):
        ...
        for rec in self:
            rec.faces_count_all = FaceidAction.search_count([("source_id", "=", rec.id)])

    @api.multi
    def _compute_unique_faces_count_today(self):
        FaceidAction = self.env["faceid.action"]
        for rec in self:
            rec.unique_faces_count_today = FaceidAction.search_count(
                [
                    ("source_id", "=", rec.id),
    ...
    def _compute_state(self):
        for rec in self:
            ...
            if len([s for s in [str(i) for i in threading.enumerate()] if name in s]):
    ...
    def save_face_odoo(self, filename, image, min_dist, face_id):
        ...
        with io.BytesIO() as buffered:
            ...
            action = self.env["faceid.action"].create(
                {"face": img_str, "name": self.name, "min_distanse": min_dist, "source_id": self.id}
            )
            self._cr.commit()
            if face_id == -1:
    ...
    def detec_face_thread(self):
        ...
        """
        for rec in self:
            listm = self.env["faceid.notification"].search([("source_id.id", "=", rec.id)])
            for notify in listm:
                ...
                try:
                    ...
                    if preprocessor.filename != "1.png":
                        ...
                        if min_dist >= threshold:
                            rec.save_face_odoo(
                                preprocessor.filename, preprocessor.scaled, min_dist, -1
                            )
                        else:
                            rec.save_face_odoo(
                                preprocessor.filename, preprocessor.scaled, min_dist, id
                            )
            ...
            with api.Environment.manage():
                ...
                try:
                    with registry(self._cr.dbname).cursor() as cr:
                        ...
                        for rec in env["faceid.source"].search([("id", "in", ids)]):
                            rec._cr.commit()
                            ...
                            while recall:
                                ...
                                if not rec.url:
                                    ...
                                    if ok:
                                        ...
                                        rec.width_source = width
                                        rec.height_source = height
                                    rec._cr.commit()
                                if camera.isOpened():
                                    recall = rec.detection_and_recognize(camera, attempts)
                                    rec._cr.commit()
                                    attempts -= 1
                                else:
                                    _logger.warning(
                                    ...
                                    camera.release()
                                    attempts -= 1
                                    _logger.warning("attempts: " + str(attempts))
                                    # give the camera some time to recover
                                    time.sleep(5)
                                    continue
                except Exception as e:
                    _logger.warning("New cursor of thread closed")
                    _logger.info("Error on line {}".format(sys.exc_info()[-1].tb_lineno))
                    _logger.warning(str(e))
                    self._cr.close()
                    # self.env.cr.close()
