...
class PreProcessor():
    ...
    def align(self,image, _logger, margin=44, image_size=160):
        img = image
        #img = img[:,:,0:3]
        bounding_boxes, _ = detect_face.detect_face(img, self.minsize, self.pnet, self.rnet, self.onet, self.threshold, self.factor, _logger)
        ...
        self.scaled = cv2.resize(cropped, (image_size, image_size), interpolation=cv2.INTER_LINEAR)
