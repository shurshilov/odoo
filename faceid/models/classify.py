...
class Classify():
    def __init__(self, _logger):
        ...
        self.odoo_path = '\\'.join(os.path.abspath(__file__).split('\\')[:-1])+"\\"
        ...
        #try:
           ...
           #with tf.Session() as sess:
                ...
                for image in images_list:
                    real_images.append(self.load_images(io.BytesIO(base64.b64decode(image))))
                #real_images.append(self.load_images(image_compare))
                real_images.append(self.load_images(self.array_to_image(image_compare)))
                ...
                for j in range(nrof_images-1):
                    dist = np.sqrt(np.sum(np.square(np.subtract(self.emb_array[nrof_images-1,:], self.emb_array[j,:]))))
                    if dist < min_dist:
                        min_dist = dist
                        id = list_ids[j]
                return min_dist, id
    ...
    def predict_db(self, path, db_path):
        ...
        self.feed_dict = { self.images_placeholder:real_images, self.phase_train_placeholder:False }
    ...
    def load_model(self, model="./model"):
        ...
        self.saver = tf.compat.v1.train.import_meta_graph(os.path.join(model_exp, meta_file), input_map=None)
        self.saver.restore(tf.compat.v1.get_default_session(), os.path.join(model_exp, ckpt_file))
