...
class Channel(models.Model):
    ...
    def _prepare_firebase_notifications(self, message, device_ids):
        ...
        {'id': 2323, 'body': '<p>123</p>', 'date': datetime.datetime(2020, 11, 19, 19, 41, 48), 'author_id': (47, 'demo1'),
        'email_from': '"demo1" <notfound@gmail.com>', 'message_type': 'comment', 'subtype_id': (1, 'Discussions'),
         'subject': False, 'model': 'mail.channel', 'res_id': 30, 'record_name': 'demo1, Artem Shurshilov',
          'channel_ids': [30], 'partner_ids': [], 'starred_partner_ids': [], 'moderation_status': 'accepted',
          'customer_email_status': 'sent', 'customer_email_data': [], 'attachment_ids': [], 'tracking_value_ids': [],
           'needaction_partner_ids': [], 'is_note': False, 'is_discussion': True, 'is_notification': False,
           'subtype_description': False, 'module_icon': '/mail/static/description/icon.png'}
        """
        message_json = {
    ...
    def _mail_channel_firebase_notifications(self, message, device_ids):
        ...
        if len(device_ids) > 1:
            data = {
                ...
                "data": {"channel_ids": message["channel_ids"], "body_html": message["body_html"]},
...
class ResUsersFirebase(models.Model):
    ...
    mail_firebase_tokens = fields.One2many(
        "mail.firebase", "user_id", string="Mail Firebase tokens", readonly=True
    )
