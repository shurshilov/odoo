  ...
  CalendarRenderer.include({
    _renderCount: function () {
      ...
      var getDaysArray = function (start, end) {
        ...
        if (!end) arr.push(new Date(start));
        return arr;
      };

      // Loop every event
      for (let i = 0; i < self.state.data.length; i++) {
        let listDates = getDaysArray(
          self.state.data[i].r_start,
          self.state.data[i].r_end,
        );
          ...
          if (cell.length > 1)
            if ($("#" + id).length) {
              ...
              let html =
                '<span id="' +
                id +
                '"class="fc-day-number" style="color:red">(' +
                nextVal +
                ")</span>";
              if (cell.hasClass("fc-today"))
                html =
                  '<span id="' +
                  id +
                  '"class="fc-day-number" style="color:red;background-color:transparent !important">(' +
                  nextVal +
                  ")</span>";
              $("#" + id).replaceWith(html);
            } else {
              let html =
                '<span id="' +
                id +
                '"class="fc-day-number" style="color:red">(1)</span>';
              if (cell.hasClass("fc-today"))
                html =
                  '<span id="' +
                  id +
                  '"class="fc-day-number" style="color:red;background-color:transparent !important">(1)</span>';
