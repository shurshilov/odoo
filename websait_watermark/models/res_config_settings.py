...
class Extension(http.Controller):
    ...
    def content_image(
        ...
        **kwargs
        ...
        if field == "image_1024" and (
            model == "product.template" or model == "product.product"
        ):
            if (
                request.env["ir.config_parameter"]
                .sudo()
                .get_param("website_watermark_enable")
            ):
                if model == "product.template":
                    field = "watermark_image"
                else:
                    field = "watermark_image_product"
        return main.Binary().content_image(
            xmlid=xmlid,
            model=model,
            id=id,
            field=field,
...
class ResConfigSettings(models.TransientModel):
    ...
    def get_default_website_watermark_enable(self):
        website_watermark_enable = self.env["ir.config_parameter"].get_param(
            "website_watermark_enable"
        )
        return website_watermark_enable

    @api.model
    def get_default_website_watermark_image(self):
        website_watermark_image = self.env["ir.config_parameter"].get_param(
            "website_watermark_image"
        )
        return website_watermark_image

    @api.model
    def get_default_website_watermark_centr(self):
        website_watermark_centr = self.env["ir.config_parameter"].get_param(
            "website_watermark_centr"
        )
        return website_watermark_centr

    @api.model
    def get_default_website_watermark_transparent(self):
        website_watermark_transparent = self.env[
            "ir.config_parameter"
        ].get_param("website_watermark_transparent")
    ...
    website_watermark_image = fields.Binary(
        "Image for watermark", default=get_default_website_watermark_image
    )
    website_watermark_text = fields.Char(
        "Text for watermark, image will default"
    )
    website_watermark_enable = fields.Boolean(
        "Enable/Disable website watermark",
        default=get_default_website_watermark_enable,
    )
    website_watermark_centralize = fields.Boolean(
        "centralize watermark or leave in the upper left corner",
        default=get_default_website_watermark_centr,
    )
    website_watermark_transparent = fields.Integer(
        "transparent watermark 0-256 opacity,0 - disable",
        default=get_default_website_watermark_transparent,
    )
    ...
    def set_website_watermark(self):
        ...
        config_parameters.set_param(
            "website_watermark_enable", self.website_watermark_enable
        )
        config_parameters.set_param(
            "website_watermark_image", self.website_watermark_image
        )
        config_parameters.set_param(
            "website_watermark_centr", self.website_watermark_centralize
        )
        config_parameters.set_param(
            "website_watermark_transparent", self.website_watermark_transparent
        )
    ...
    def write(self, values):
        ...
        if self.website_watermark_image and self.website_watermark_enable:
            watermark = Image.open(
                io.BytesIO(base64.b64decode(self.website_watermark_image))
            ).convert("RGBA")
            if (
                self.website_watermark_transparent
                and self.website_watermark_transparent < 256
                and self.website_watermark_transparent > 0
            ):
                watermark.putalpha(self.website_watermark_transparent)
            for prod in self.env["product.template"].search([]):
                if prod.image_1024:
                    img = Image.open(
                        io.BytesIO(base64.b64decode(prod.image_1024))
                    ).convert("RGBA")
                    # x, y = watermark.size
                    ...
                    with io.BytesIO() as output:
                        ...
                        ) if img.format else img.save(output, format="PNG")
                        # transparent.save(output)
                        prod.watermark_image = base64.b64encode(
                            output.getvalue()
                        )
            for prod in self.env["product.product"].search([]):
                if prod.image_1024:
                    ...
                    with io.BytesIO() as output:
                        ...
                        # transparent.save(output)
                        prod.watermark_image_product = base64.b64encode(
                            output.getvalue()
                        )
        return result
